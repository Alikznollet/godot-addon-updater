...
## Windows (via Scoop)
If you use [Scoop](https://scoop.sh/), you can add the custom bucket and install `wisp` instantly:
```powershell
...
If you use [Homebrew](https://brew.sh/), you can tap the repository and install `wisp` via the terminal:
```bash
# Tap the repository
...
## Updating 
Because `wisp` is installed via your package manager, keeping it up to date is completely frictionless.
